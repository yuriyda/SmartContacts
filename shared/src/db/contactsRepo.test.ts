...
describe('contactsRepo', () => {
  ...
  // P7.T1: bulkLoad + read-back preserves protected and hidden flags
  test('bulkLoad preserves protected and hidden flags', async () => {
    const db = await fresh('contacts-repo-10')
    const repo = makeContactsRepo(db, 'DEV')
    const c1 = newContact({ displayName: 'Protected One', protected: true })
    const c2 = newContact({ displayName: 'Hidden One', hidden: true })
    const c3 = newContact({ displayName: 'Both Flags', protected: true, hidden: true })
    const c4 = newContact({ displayName: 'No Flags' })
    await repo.bulkLoad([c1, c2, c3, c4])
    const r1 = await repo.getById(c1.id)
    const r2 = await repo.getById(c2.id)
    const r3 = await repo.getById(c3.id)
    const r4 = await repo.getById(c4.id)
    expect(r1?.protected).toBe(true)
    expect(r1?.hidden).toBeUndefined()
    expect(r2?.hidden).toBe(true)
    expect(r2?.protected).toBeUndefined()
    expect(r3?.protected).toBe(true)
    expect(r3?.hidden).toBe(true)
    expect(r4?.protected).toBeUndefined()
    expect(r4?.hidden).toBeUndefined()
    await db.close()
  })
})
